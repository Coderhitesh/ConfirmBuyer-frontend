      ...
      <div className="w-3/4  grids h-full p-4">
        {Products &&
          Products.slice(0,8).map((item, index) => (
            <div
              key={index}
              className="bg-white p-4 m-2 borders glass2 rounded-lg hover:shadow-lg transition duration-300 h-[280px] flex flex-col justify-between text-center border border-solid"
            >
              <Link
                to={`/product/${item._id}`}
